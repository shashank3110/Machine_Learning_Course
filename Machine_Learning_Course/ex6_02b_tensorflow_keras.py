...
def create_neural_net_keras(X_train,y_train,X_test,y_test,batch_size,epochs=1):
    ...
    tensorboard_callback=tf.keras.callbacks.TensorBoard('logs_2b_tf_keras',write_graph=True,batch_size=batch_size) 
    model=Sequential()
...
model=create_neural_net_keras(train_images,train_labels,test_images,test_labels,batch_size=183,epochs=10)
